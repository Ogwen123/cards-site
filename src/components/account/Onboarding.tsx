...
const Onboarding = () => {
    ...
    const submit = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (formData.get("password") !== formData.get("confirm-password")) {
            setAlertData(["Passwords must match.", true, "ERROR"])
            setTimeout(() => {
                setAlertData(["", false, "NONE"])
            }, config.alertLength)
            return
        ...
        const res = await fetch(getApiUrl() + "auth/onboarding", {
            ...
            body: JSON.stringify({
                email: email,
                code: code,
                username: formData.get("idfkgoaway")?.toString()!,
        ...
        if (!res.ok) {
            ...
            setAlertData(["Account Successfully Onboarded.", true, "SUCCESS"])
            setLoading(false)
            setTimeout(() => {
                setAlertData(["", false, "NONE"])
            }, 2000)
    ...
    if (uriParams.hasParams(["email", "code"])) {
        return (
            <div className="text-lg fc flex-col mt-[100px]">
                <Alert message={alertData[0]} show={alertData[1]} severity={alertData[2]} />
                <div className='text-2xl my-5'>Finalise your account by creating a username</div>
                <form className='fc flex-col form'>
                    <input name="idfkgoaway" type="text" placeholder='Create Username' className='form-input' />
                    <input name="password" type="password" placeholder='Password' className='form-input' />
                    <input name="confirm-password" type="password" placeholder='Confirm Password' className='form-input' />
        ...
        return (
            <Error message={["Incorrect URL parameters.", "You should only access this link from the email you were sent.", "If you are getting this error from the email link then please try registering again."]} />
        )
    }
}
